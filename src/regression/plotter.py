...
def run_binary_and_capture_output():
    ...
    process = subprocess.Popen(['./regression/main'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    with open('./regression/regression_dataset.csv', 'r') as infile:
...
def plot_training_loss(training_loss_values):
    ...
    plt.show()
    plt.savefig("./regression/graph.png")
