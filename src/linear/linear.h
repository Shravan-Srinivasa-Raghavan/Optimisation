...
#define LINEAR
#include "../matrix.h" // This imports the file which has all the definitions regarding matrices. 
#include <random> 
#include <algorithm>
...
class LinearRegression{
    private:
        ...
        vector<float> train_loss; // Stores the loss from train data after every 100th iteration in gradient descent
    ...
    public:
        ...
        */
        ...
        float l2loss(matrix X, matrix Y); // Finds the mean squared loss given the input data and the true output values
        pair<matrix,float> l2lossDerivative(matrix X, matrix Y); // Computes the gradient given input data and true output values and returns dL/dw and dL/db
        void GD(matrix X, matrix Y, float learing_rate, uint64_t limit); // Performs gradient descent given the input data, true output values,learning rate and a maximum limit for iterations
        void train(matrix X_train, matrix Y_train, float learning_rate, uint64_t limit); // This function calls gradient descent
        ...
        float accuracy(matrix,matrix); // Computes the accuracy of the model as defined in README.md
};
