...
pair<matrix,matrix> parser(std::istream &input){
    ...
    vector<vector<float>> X_vec;
    vector<float> Y_vec;
    string line;
    while(getline(input,line)){
        vector<float> tokens=split(line,',');
...
pair<matrix, matrix> generate_linear_data(uint64_t n, uint64_t d) {
    ...
    for (uint64_t j = 0; j < d; j++) {
        w(j, 0) = static_cast<float>(rand()) / RAND_MAX;
    }

    float b = static_cast<float>(rand()) / RAND_MAX; // Bias term

    for (uint64_t i = 0; i < n; i++) {
        float y = b;
        for (uint64_t j = 0; j < d; j++) {
            X(i, j) = static_cast<float>(rand()) / RAND_MAX;
            y += X(i, j) * w(j, 0);
        }
        y += static_cast<float>(rand()) / RAND_MAX * 0.1; // Adding some noise
        Y(i, 0) = y;
    }
